...
PROCEDURE Lab6 IS
   ...
   PROCEDURE Compute_Charges (Num_Pizzas_FP : IN Integer;
      ...
      Osteopathic_Charge_FP : IN OUT Float;
      Count: IN Float) IS
   ...
   BEGIN
      ...
      when 4
         =>Bulldog_Charge_FP:= 8.0 * Float(Num_Pizzas_FP) -6.0;
      when 5
   ...
   Osteopathic_Charge :  Float:=        6.0;  --Charges  for Bulldog Pizza and Osteopathic Pizzas
   Header             : Integer;

   -----------------------------------------------------------------------------------
BEGIN --Lab6 (begin of main program)
   Get(Header);
